...
export class CubismModel {
  ...
  public getRenderOrders(): Int32Array {
    const modelWithLegacyFallback = this._model as unknown as {
  ...
  public getDrawableColorBlend(drawableIndex: number): CubismColorBlend {
    const drawableBlendModes = (this._model.drawables as { blendModes?: Int32Array })
      .blendModes;

    if (
      !drawableBlendModes ||
      drawableIndex < 0 ||
      drawableIndex >= drawableBlendModes.length
    ) {
    ...
    if (
      ...
      this._drawableColorBlends[drawableIndex] =
        drawableBlendModes[drawableIndex] & 0xff;
    }
  ...
  public getDrawableAlphaBlend(drawableIndex: number): CubismAlphaBlend {
    ...
    if (
      ...
      return CubismAlphaBlend.AlphaBlend_Over;
    }
    ...
    if (
      ...
      this._drawableAlphaBlends[drawableIndex] =
        (drawableBlendModes[drawableIndex] >> 8) & 0xff;
    }
  ...
  public getOffscreenColorBlend(offscreenIndex: number): CubismColorBlend {
    const offscreenBlendModes = (this._model.offscreens as {
      blendModes?: Int32Array;
    }).blendModes;
    ...
    if (
      !offscreenBlendModes ||
      offscreenIndex < 0 ||
      offscreenIndex >= offscreenBlendModes.length
    ) {
      return CubismColorBlend.ColorBlend_Normal;
    }
    ...
    if (
      ...
      this._offscreenColorBlends[offscreenIndex] =
        offscreenBlendModes[offscreenIndex] & 0xff;
    }
  ...
  public getOffscreenAlphaBlend(offscreenIndex: number): CubismAlphaBlend {
    ...
    if (
      ...
      this._offscreenAlphaBlends[offscreenIndex] =
        (offscreenBlendModes[offscreenIndex] >> 8) & 0xff;
    }
  ...
  public constructor(model: Live2DCubismCore.Model) {
    // 旧Coreではoffscreens自体が未定義のため、空配列構造を補って初期化を継続する。
    ...
    this._model = mutableModel as unknown as Live2DCubismCore.Model;
